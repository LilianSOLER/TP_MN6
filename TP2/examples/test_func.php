...
$DIV_CSV = ["float" => "div_float.csv", "double" => "div_float.csv"];

function test_div_complexe($tests){
  echo "Test div_complexe() ...\n\n";
  ...
  foreach($tests as $title => $file){
    ...
    $max_max_diff = 0;
    foreach($arr as $calc){
      $c3 = div_complexe($calc["c1"], $calc["c2"]);
      $max_diff = max(abs($c3["re"] - $calc["c3"]["re"]), abs($c3["re"] - $calc["c3"]["re"]));
      if($max_max_diff < $max_diff){
        $max_max_diff = $max_diff;
      }
