...
const int max = (incX < incY) ? ceil((float)N / (float)incY) : ceil((float)N / (float)incX);
  ...
  for (register unsigned int i = 0; i < max; i++)
  {
    dotu_tmp[0].real += X_tmp[i].real * Y_tmp[diff*i].real - X_tmp[i].imaginary * Y_tmp[diff*i].imaginary;
    dotu_tmp[0].imaginary += X_tmp[i].real * Y_tmp[diff*i].imaginary + X_tmp[i].imaginary * Y_tmp[diff*i].real;
  }
  dotu = (void *)dotu_tmp;
...
const int max = (incX < incY) ? ceil((float)N / (float)incY) : ceil((float)N / (float)incX);
  ...
  for (register unsigned int i = 0; i < max; i++)
  {
    dotc_tmp[0].real += X_tmp[i].real * Y_tmp[diff*i].real + X_tmp[i].imaginary * Y_tmp[diff*i].imaginary;
    dotc_tmp[0].imaginary += -(X_tmp[i].real * Y_tmp[diff*i].imaginary) + X_tmp[i].imaginary * Y_tmp[diff*i].real;
  }
  dotc = (void *)dotc_tmp;
